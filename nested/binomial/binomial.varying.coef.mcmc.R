...
binomial.varying.coef.mcmc <- function(z,N,X,g,priors,start,tune,adapt=TRUE,n.mcmc=1000){
...
# browser()
	...
	p <- sapply(1:J,function(x) expit(X[g.idx[[x]],]%*%beta[,x]))  # binomial probability
...
# browser()
		...
		for(i in 1:J){
			...
			p.star <- expit(X[idx,]%*%beta.star)  # intensity of Poisson process
	  		mh.0 <- sum(dbinom(z[idx],N[idx],p[,i],log=TRUE))+
...
# browser()		
	...
	list(beta=beta.save,mu.beta=mu.beta.save,Sigma=Sigma.save,keep=keep,
		z=z,X=X,N=N,g=g,priors=priors,start=start,tune=tune,n.mcmc=n.mcmc)
}